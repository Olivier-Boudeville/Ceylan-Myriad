...
-export([ start_main_event_loop/2, propagate_event/1, wx_to_myriad_event/1,
...
-type event_subscription_option() ::
	% Tells, once a corresponding event will have been processed, to still
	% propagate it upward in the widget hierarchy, further event handlers being
	% then triggered (the default being not to propagate an event once
	% processed).
	'propagate_event'.
...
-type event_subscription() ::
	  ...
	  | { maybe_list( event_type() ), maybe_list( gui_object() ),
		  maybe_list( event_subscriber_pid() ) }
...
process_event_message( { subscribeToEvents,
	...
	cond_utils:if_defined( myriad_debug_gui_events,
		trace_utils:debug_fmt( "[event] Subscribing by ~w of process ~w "
			"to events ~w.",
			[ SenderPid, SubscriberPid, SubscribedEvents ] ) ),
...
% Wanting to have the actual user-defined identifiere (directly) available in
% the message sent back:
%
send_event( Subscribers, _EventType=onMenuItemSelected, EventSourceId,
			GUIObject, UserData, Event ) ->

	% Integrates the lower-level backend identifier:
	Context = #event_context{ id=EventSourceId, user_data=UserData,
							  backend_event=Event },

	% Translating if possible this backend identifier into a higher-level name:
	BestId = gui_id:try_resolve_id( EventSourceId ),

	Msg = { onMenuItemSelected, [ GUIObject, BestId, Context ] },
...
register_in_event_loop_tables( _SubscribedEvents=[
	...
	% Just adding the default subscriber and option to have a subscription
	% quadruplet:
	%
	register_in_event_loop_tables( [ { EventTypeMaybeList, GUIObjectMaybeList,
		[ DefaultSubscriberPid ], _SubscriptionOpts=[] } | T ],
		DefaultSubscriberPid, LoopState );
...
register_in_event_loop_tables( _SubscribedEvents=[
		...
		DefaultSubscriberPid, LoopState ) ->
	% Just adding the default option to have a subscription quadruplet:
	register_in_event_loop_tables( [ { EventTypeMaybeList, GUIObjectMaybeList,
		SubscriberMaybeList, _SubscriptionOpts=[] } | T ], DefaultSubscriberPid,
		LoopState );

% Full, canonical subscription quadruplet:
register_in_event_loop_tables( _SubscribedEvents=[
		{ EventTypeMaybeList, GUIObjectMaybeList, SubscriberMaybeList,
		  SubscriptionMaybeOpts } | T ],
		DefaultSubscriberPid, LoopState ) ->
	...
	SubOptList     = list_utils:ensure_proplist( SubscriptionMaybeOpts ),
...
-spec register_event_types_for( gui_object(), [ event_type() ],
			[ event_subscriber_pid() ], [ connect_opt() ], loop_state() ) ->
											loop_state().
register_event_types_for( Canvas={ myriad_object_ref, myr_canvas, CanvasId },
						  EventTypes, Subscribers, SubOptList,
						  LoopState=#loop_state{ event_table=EventTable,
												 type_table=TypeTable } ) ->
...
register_event_types_for( GUIObject, EventTypes, Subscribers, SubOptList,
						  LoopState=#loop_state{ event_table=EventTable } ) ->
	...
	%
	[ gui_wx_backend:connect( GUIObject, EvType, SubOptList )
						|| EvType <- EventTypes ],
...
unregister_event_types_from( Canvas={ myriad_object_ref, myr_canvas, CanvasId },
		EventTypes, Unsubscribers, LoopState=#loop_state{
			event_table=EventTable, type_table=TypeTable } ) ->
