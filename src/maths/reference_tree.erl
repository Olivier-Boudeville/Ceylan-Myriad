...
% "pointer" to the parent one (if any), and a transformation (whose reference
% matrix is the transition matrix from the node to its parent, and whose inverse
...
% namely R1, and then to go from R1 to C2. Pros: most direct computations. Cons:
% no easy reuse of them.
%
...
% like here for R2 and R1 (no runtime overhead incurred, but possibly more
% rounding errors). Pros: then each node can (optionally) store, in addition to
% the parent transform, a root one. We could then obtain the transform from R3
% to C2 thanks to only one matrix multiplication. The root node would then have
% to record up to one transformation for each of the other nodes.
%
% In both cases we should precompute or cache the paths involved for each pair
% of endpoints, as a triplet made of a "up moves" list and then of "down moves":
% and then any already computed transform:
%
% ref_path() :: {UpMoves :: [ref_designator()], DownMoves :: [ref_designator()],
% maybe(transform())}.
%
% For example to reach C2 from R3, the shortest path would be: PAs={[R2,R1],[]}
% (meaning R3 -> R2 -> R1 -> C2), while the path through root would be:
% PAr={[R2,R1, Root],[R1]}.
%
% Either way such a path_table({From :: ref_designator(), To ::
% ref_designator()}) -> ref_path() could store the known paths.
%
...
% dedicated table(ref_designator(), [path_id()]) to invalidate the corresponding
% paths easily.
%
...
% ref_designator(), To :: ref_designator()) -> transform().
...
-record( reference_tree, {
	...
	% The table of this tree concentrating all known reference frames.
	ref_table :: ref_table(),
...
register( Ref3=#reference_frame3{ parent=MaybeParent },
	...
	NewChildTable = case MaybeParent of
		...
		undefined ->
			table:add_new_entry( Id, _Children=[], ChildTable );
...
% @doc Returns a textual representation of the specified reference tree.
