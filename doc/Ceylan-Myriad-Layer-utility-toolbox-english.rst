...
.. [#] To have such dependencies (ex; ``jsx``) *installed* as well when building one's project (ex: ``foobar``), one may list them in the project's ``conf/rebar.config.template`` file (ex: ``{deps, [foo, bar, jsx]}.``) from which the actual ``rebar.config`` is to be generated (use ``make set-rebar-conf`` for that). Such dependencies may or may not be listed in the ``deps`` entry of the  ``conf/foobar.app.src`` file (if editing it, run ``make rebar3-create-app-file`` afterwards in order to have the three other versions of it properly generated).
